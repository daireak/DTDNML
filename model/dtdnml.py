...
class DTDNML(BaseModel):
    ...
    def initialize(
        ...
        # 修改后:
        H, W = int(lrhsi_hei * self.opt.scale_factor), int(lrhsi_wid * self.opt.scale_factor)
        # 动态获取 HR 图像的尺寸，而不是写死 500
        code_tensor_scale = [H, W, num_s]
        # 分别使用 hei 和 wid，支持非正方形输入
        code_tensor_scale_lr = [int(lrhsi_hei), int(lrhsi_wid), num_s]
    ...
    def my_backward_g_joint(self, epoch):
        ...
        # ===============================================================
        # 兼容单卡/多卡模式，动态脱去 DataParallel 的 .module 壳
        # ===============================================================
        lr_hsi_wh_net = self.lr_hsi_dict_wh.module if hasattr(self.lr_hsi_dict_wh, 'module') else self.lr_hsi_dict_wh
        lr_hsi_s_net = self.lr_hsi_dict_s.module if hasattr(self.lr_hsi_dict_s, 'module') else self.lr_hsi_dict_s
        hr_msi_wh_net = self.hr_msi_dict_wh.module if hasattr(self.hr_msi_dict_wh, 'module') else self.hr_msi_dict_wh
        hr_msi_s_net = self.hr_msi_dict_s.module if hasattr(self.hr_msi_dict_s, 'module') else self.hr_msi_dict_s

        h = lr_hsi_wh_net.conv_h.weight.permute(2, 3, 0, 1).squeeze()
        w = lr_hsi_wh_net.conv_w.weight.permute(2, 3, 0, 1).squeeze()
        V = lr_hsi_s_net.conv_s.weight.permute(2, 3, 0, 1).squeeze()
        ...
        H = hr_msi_wh_net.conv_h.weight.permute(2, 3, 0, 1).squeeze()
        W = hr_msi_wh_net.conv_w.weight.permute(2, 3, 0, 1).squeeze()
        v = hr_msi_s_net.conv_s.weight.permute(2, 3, 0, 1).squeeze()
