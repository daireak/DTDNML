...
class FeatureUNet(nn.Module):
    ...
    def forward(self, hr_msi_feature, lr_hsi_feature, epoch, transformer=False):
        ...
        if not transformer:
            ...
            feature_downsample_1 = ((feature_downsample_1.squeeze().reshape([c, h*w]).T) @ spe_attn_1.squeeze()).T.reshape(c, h, w).unsqueeze(0)

        feature_downsample_1_next_in = self.dowmsample_1(feature_downsample_1)

        # downsample 2
        feature_downsample_2 = self.relu(feature_downsample_1_next_in + self.down_sample_conv_list_2(feature_downsample_1_next_in))
        ...
        if not transformer:
            ...
            feature_downsample_2 = ((feature_downsample_2.squeeze().reshape([c, h*w]).T) @ spe_attn_2).T.reshape(c, h, w).unsqueeze(0)

        feature_downsample_2_next_in = self.dowmsample_2(feature_downsample_2)

        # ===============================================================
        # 🌟 核心修复：根据 scale_factor 动态控制 U-Net 深度 (兼容 4x 和 8x)
        # ===============================================================
        if self.iterations == 2:
            # 【4倍缩放】U-Net 只有两层，此时直接进入 bottom 层
            feature_bottom = self.bottom_layer(
                torch.cat([feature_downsample_2_next_in, lr_hsi_feature_iter_2], dim=1)
            )
            # 直接将 bottom 传给第二层上采样作为输入，跳过第三层的处理
            upsample_feature_3 = feature_bottom
            transformer = True
            ...
            if not transformer:
                ...
                # Transformer to extract channel attention
                lr_hsi_feature_iter_3, spe_attn_3 = self.ssab3(lr_hsi_feature_iter_3) 
        ...
        #     # save the feature map
        #     import scipy.io as sio
        #     save_mat = {"spatial_out_2":spatial_out_2.data.cpu().float().numpy()}
        #     sio.savemat(r"D:\\Dataset\\tensor_factorization\\CrossAttention\\checkpoints\\pavia_scale_8\\feature_map_1229\\spatial_attn2_{}.mat".format(epoch), 
        #                 save_mat)
        ...
        if transformer:
            ...
            upsample_feature_2 = ((upsample_feature_2.squeeze().reshape([c, h*w]).T) @ spe_attn_2).T.reshape(c, h, w).unsqueeze(0)
        # concat the features with spatial attention
        ...
        #     # save the feature map
        #     import scipy.io as sio
        #     save_mat = {"upsample_feature_2":upsample_feature_2.data.cpu().float().numpy()}
        #     sio.savemat(r"D:\\Dataset\\tensor_factorization\\CrossAttention\\checkpoints\\pavia_scale_8\\feature_map_1229\\feature_map2_{}.mat".format(epoch), 
        #                 save_mat)
        ...
        #     # save the feature map
        #     import scipy.io as sio
        #     save_mat = {"spatial_out_1":spatial_out_1.data.cpu().float().numpy()}
        #     sio.savemat(r"D:\\Dataset\\tensor_factorization\\CrossAttention\\checkpoints\\pavia_scale_8\\feature_map_1229\\spatial_attn1_{}.mat".format(epoch), 
        #                 save_mat)
        ...
        if transformer:
            ...
            upsample_feature_1 = ((upsample_feature_1.squeeze().reshape([c, h*w]).T) @ spe_attn_1).T.reshape(c, h, w).unsqueeze(0)
        # concat the features with spatial attention
...
class LrHSIDictionaryWH(nn.Module):
    def __init__(self, code_scale, hsi_scale_w, hsi_scale_h):
        ...
        # 修复: code_scale[0] 是 H，应映射到 hsi_scale_h
        self.conv_h = nn.Conv2d(code_scale[0], hsi_scale_h, kernel_size=1, stride=1, padding=0, bias=False)
        self.conv_w = nn.Conv2d(code_scale[1], hsi_scale_w, kernel_size=1, stride=1, padding=0, bias=False)
        self.relu = nn.ReLU()
...
class LrHSIDictionaryWHT(nn.Module):
    def __init__(self, code_scale, hsi_scale_w, hsi_scale_h):
        ...
        self.conv_h = nn.Conv2d(hsi_scale_h, code_scale[0], kernel_size=1, stride=1, padding=0, bias=False)
        self.conv_w = nn.Conv2d(hsi_scale_w, code_scale[1], kernel_size=1, stride=1, padding=0, bias=False)
        self.relu = nn.ReLU()
...
class HrMSIDictionaryWH(nn.Module):
    def __init__(self, code_scale, msi_scale_w, msi_scale_h):
        ...
        self.conv_h = nn.Conv2d(code_scale[0], msi_scale_h, kernel_size=1, stride=1, padding=0, bias=False)
        self.conv_w = nn.Conv2d(code_scale[1], msi_scale_w, kernel_size=1, stride=1, padding=0, bias=False)
...
class HrMSIDictionaryWHT(nn.Module):
    def __init__(self, code_scale, msi_scale_w, msi_scale_h):
        ...
        self.conv_h = nn.Conv2d(msi_scale_h, code_scale[0], kernel_size=1, stride=1, padding=0, bias=False)
        self.conv_w = nn.Conv2d(msi_scale_w, code_scale[1], kernel_size=1, stride=1, padding=0, bias=False)
